...
- [Getting Started](#getting-started)
  - [1. Clone/Fork the Repository](#1-clonefork-the-repository)
  - [2. Rename Project Identity](#2-rename-project-identity)
  - [3. Update Package Name](#3-update-package-name)
  - [4. Configure Flavors](#4-configure-flavors)
  - [5. Setup Firebase (Optional)](#5-setup-firebase-optional)
...
Follow these steps to set up this template for your new project:
...
```yaml
name: your_project_name        # Line 1: Change this to your project name (snake_case)
description: Your project description.  # Line 2: Update description
version: 1.0.0+1               # Line 5: Set your initial version
```

**Example:**
```yaml
name: my_awesome_app
description: An amazing Flutter application.
version: 1.0.0+1
```
...
**a. build.gradle.kts** (`android/app/build.gradle.kts`)

Update lines 4-6:
```kotlin
// TODO: change app-name & app-id
val applicationId = "com.yourcompany.yourapp"      // Your package name
val applicationName = "Your App Name"              // Your app display name
```

**Example:**
```kotlin
val applicationId = "com.mycompany.awesomeapp"
...
**c. AndroidManifest.xml** (`android/app/src/main/AndroidManifest.xml`)

Update package attribute if present, or ensure the `android:label` matches your app name:
```xml
...
Update flavor-specific configurations:
...
Lines 10-11:
```dart
const _name = 'Your App Name Dev';        // Development app name
const _apiUrl = 'https://dev-api.yourcompany.com';  // Development API URL
```
...
Lines 10-11:
```dart
const _name = 'Your App Name';            // Production app name
const _apiUrl = 'https://api.yourcompany.com';      // Production API URL
```

### 5. Setup Firebase (Optional)

If you want to use Firebase features (Cloud Messaging, Analytics, etc.):

1. **Create Firebase Projects:**
   - Go to [Firebase Console](https://console.firebase.google.com/)
   - Create two projects (or one project with two apps):
...
3. **iOS Configuration:**
   ...
   - Configure schemes for different environments if needed

4. **Update Notification Channel Name:**
   **File:** `lib/shared/services/notification_service.dart` (line 52-53)
   ```dart
   const AndroidNotificationChannel channel = AndroidNotificationChannel(
     'default_notification_channel', // Keep this ID
     'Your App Name Notifications',  // Change this to your app name
     description: 'This channel is used for important notifications.',
     importance: Importance.high,
   );
   ```

**To skip Firebase:** Remove or comment out Firebase-related code in `lib/bootstrap.dart` and `pubspec.yaml`.
...
Generate code for models, API services, and assets:
...
- Add/modify JSON models with `@JsonSerializable`
- Add/modify Retrofit API services
...
- [ ] Update Android `applicationId` and `applicationName` in `build.gradle.kts`
- [ ] Rename Android package folder structure
- [ ] Update Android `MainActivity.kt` package declaration
- [ ] Update iOS `CFBundleDisplayName` and `CFBundleName` in `Info.plist`
- [ ] Update flavor names and API URLs in `main_development.dart`
- [ ] Update flavor names and API URLs in `main_production.dart`
- [ ] Update notification channel name in `notification_service.dart`
- [ ] Add Firebase configuration files (if using Firebase)
  - [ ] `google-services.json` for Android (dev & prod)
  - [ ] `GoogleService-Info.plist` for iOS
- [ ] Replace app launcher icons in `android/app/src/development/res/` and `production/res/`
- [ ] Update API endpoints in `lib/shared/constants/apis.dart`
- [ ] Run `flutter pub get`
- [ ] Run `flutter pub run build_runner build --delete-conflicting-outputs`
- [ ] Test run both flavors (development & production)
- [ ] Remove/modify example Products feature as needed
...
5. Follow the style guide (see `analysis_options.yaml`)
